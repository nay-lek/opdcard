...
<form id="form1" name="form1" method="post" action="" >
  <div id="main">
   ...
   <table width="100%" height="58" border="0" cellspacing="0" cellpadding="0" background="bar.png">
        <tr>
          <td width="100%" height="20" align="center"><div><!--<table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td >--><h3 class="fcolor">
                <?php 
				
				$sqlPt="select concat(pname,fname,' ',lname) as fullname from patient where hn='$ptHN'";
...
echo $pt['fullname'];
    ...
    <br />
      ...
      <input name="txtSearch" type="text" id="txtSearch" value="<?php echo $ptHN; ?>" />
      <input type="submit" name="button" id="button" value="ค้นหา" onclick="parent.frames[1].location.href = 'opdcardViewImage.php?visit=0'" />
     <br />
     <B>OPD</B>
      <?php
	  $_SESSION['HN']=$ptHN;
	...
	?>
      <li class="closed"><span class="folder"><?php echo intval($row->viewyear+543); ?></span>
		...
		$Cnumrow=mysql_num_rows($dbqueryChkClinic);
				if ($Cnumrow > 0){
							$sqldetail="select ov.vn,replace(ov.vstdate,'/','-') as newdate,ov.vsttime,s.name as spclty,ov.an , ov.o_refer_number as refer from ovst ov
...
limit ".LimitY;
		...
		<li class="closed"><span class="folder"><?php echo intval($result['viewyear']+543); ?></span>
			...
			?>
				<li><span class="file"><?php echo "<a href='viewchart.php?an=".$row_ipd['an']."' target='chapter'>".thai_date_short(strtotime($row_ipd['regdate']))."</a>"; ?></span></li>
            <?php } ?>
			</ul>
		</li>
        <?php } ?>
